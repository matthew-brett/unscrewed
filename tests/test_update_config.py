...
def test_check_add_repo(tmp_path):
    with pytest.raises(RepoError):
        check_add_repo(BASE_URL.replace("{version}", "0.3a1"), "0.1a2", tmp_path)
    with pytest.raises(RepoError):
        check_add_repo(BASE_URL, None, tmp_path)
    out_path, rel_path = check_add_repo(BASE_URL, "0.1a2", tmp_path)
